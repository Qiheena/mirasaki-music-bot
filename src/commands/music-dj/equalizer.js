...
module.exports = new ChatInputCommand({
  ...
  run: async (client, interaction) => {
    ...
    if (process.env.USE_LAVALINK === 'true' && client.lavalink) {
      interaction.reply(`${ emojis.error } ${ member }, equalizer presets are currently only available when using discord-player. Lavalink uses a different filter system that will be implemented in a future update.`);
      return;
    }
