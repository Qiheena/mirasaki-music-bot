  ...
  run: async (client, interaction) => {
    ...
    try {
      if (process.env.USE_LAVALINK === 'true' && client.lavalink) {
        ...
        // Swap src and dest
        [queue.tracks[firstPosition], queue.tracks[secondPosition]] = [queue.tracks[secondPosition], queue.tracks[firstPosition]];
        const firstTrack = queue.tracks[firstPosition];
        const secondTrack = queue.tracks[secondPosition];
        interaction.reply(`${ emojis.success } ${ member }, **\`${ firstTrack.info.title }\`** has been swapped with **\`${ secondTrack.info.title }\`**`);
      } else {
