...
const generateEmbed = (page, categorizedData, client, prefix) => {
  ...
  } else { // This part won't be used by home, but kept for consistency
...
module.exports = new Button({
  ...
  run: async (client, interaction) => {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('help_previous').setLabel('Previous').setStyle(ButtonStyle.Secondary).setDisabled(true),
      new ButtonBuilder().setCustomId('help_home').setLabel('Home').setStyle(ButtonStyle.Primary).setDisabled(true),
      new ButtonBuilder().setCustomId('help_next').setLabel('Next').setStyle(ButtonStyle.Secondary).setDisabled(totalPages <= 1)
    );
